...
const command: CommandModule<unknown> = {
  ...
  async handler() {
    ...
    spinner.start('Linking ring to js packages ...');
    ...
    try {
      ...
      if (await exists(linkPath)) {
        spinner.info('Already linked');
      } else {
        logger.debug`Creating symlink ${linkPath}`;
        await fs.symlink(exePath, linkPath, 'file');
        spinner.success('Link successfully created');
      }
    } catch (error) {
      spinner.stop();
      logger.error('Failed to link ring', error as Error);
    }
  }
};
